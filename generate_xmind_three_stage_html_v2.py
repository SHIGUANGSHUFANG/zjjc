...
<style>
  ...
  #questionPage {
    ...
    background:linear-gradient(180deg,#fbfbff,#f2f4f8);
  }
...
<div id="questionPage">
  <div class="question-wrap">
    <div class="question-head">
      <h1>试题练习</h1>
      <p>完成全部选择题后提交，系统会显示正确率、正误和解析；核对后可以重新做题。</p>
    </div>
...
function shift(node, dy) {
  node.y += dy;
  (node.children || []).forEach(c => shift(c, dy));
}
...
function shiftSubtree(node, dy) {
  node.y += dy;
  (node.children || []).forEach(c => shiftSubtree(c, dy));
}
...
function updateToolbar() {
  ...
  document.getElementById('hint').textContent = preview
    ? '先看完整导图；学习结束后点击“记住了”进入挖空练习。'
    : (quiz
      ? '点击浅色空格块选择答案；提交核对后才会出现“进入试题”。'
      : '完成全部选择题后提交，系统会显示答案和解析。');
}
...
function render(keepView=false, heightPass=0) {
  ...
  for (const n of positioned) {
    ...
    const notes = (n.notes || [])
      .map(t => `<div class="note">${renderClozeText(t)}</div>`)
      .join('');

    const imgs = (n.images || [])
      .map(img => `<img src="${img.data_url}" alt="插图"/>`)
      .join('');
...
function startQuiz() {
  ...
  try {
    history.replaceState({phase:'quiz'}, '', location.href);
    history.pushState({phase:'quiz'}, '', location.href);
  } catch(e) {}
...
function openOptions(blank) {
  ...
  menu.innerHTML = options
    .map(o => `<button type="button" data-v="${escapeHTML(o)}">${escapeHTML(o)}</button>`)
    .join('');

  menu.querySelectorAll('button').forEach(btn => {
    btn.onclick = () => chooseOption(blank, btn.dataset.v);
  });
...
function chooseOption(blank, value) {
  ...
  if (nxt && nxt.classList && nxt.classList.contains('answer-inline')) {
    nxt.remove();
  }
...
function submitAnswers() {
  ...
  document.querySelectorAll('.blank').forEach(b => {
    ...
    if (old && old.classList && old.classList.contains('answer-inline')) {
      old.remove();
    }
  ...
  typesetMath().then(() => {
    requestAnimationFrame(() => {
      if (mode !== 'questions' && updateMeasuredHeights()) {
        render(true, 0);
      }
    });
  });
...
function renderQuestions() {
  ...
  list.innerHTML = questions.map((q, qi) => {
    const opts = shuffle(q.options || []).map((text, oi) => ({
      text,
...
function submitQuestions() {
  ...
  if (missing.length) {
    alert(`还有 ${missing.length} 道题没有作答。`);
    return;
  }
...
function zoomBy(k) {
  scale = Math.max(.2, Math.min(2.5, scale * k));
  applyTransform();
}
...
vp.addEventListener('mousedown', e => {
  if (e.target.closest('.blank') || e.target.closest('#optionMenu') || e.target.closest('.toolbar')) {
    return;
  }
