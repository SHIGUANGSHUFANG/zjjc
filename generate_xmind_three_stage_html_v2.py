...
<style>
  ...
  .node.root {
    background:linear-gradient(135deg,#202236,#3b3f63);
    color:white;
    text-align:center;
    border:none;
  ...
  .node-inner { padding:14px 16px; box-sizing:border-box; }
  ...
  .notes { margin-top:10px; display:flex; flex-direction:column; gap:9px; }
  ...
  .level3 .note { font-size:15px; line-height:1.76; }
  ...
  #optionMenu mjx-container,
  .blank mjx-container,
  .answer-inline mjx-container {
  ...
  #optionMenu mjx-container[display="true"],
  .blank mjx-container[display="true"],
  .answer-inline mjx-container[display="true"] {
    display:block;
    margin:.45em auto;
    overflow-x:auto;
    overflow-y:hidden;
  ...
  .q-stem { font-size:16px; line-height:1.75; color:#1f2937; margin-bottom:12px; }
  .q-option { display:flex; align-items:flex-start; gap:8px; padding:10px 12px; border:1px solid #edf1f7; border-radius:12px; background:#f8fafc; margin:8px 0; cursor:pointer; line-height:1.7; }
  ...
  .q-analysis { display:none; margin-top:12px; padding:12px 14px; border-radius:12px; background:#fff7ed; border:1px solid #fed7aa; color:#7c2d12; line-height:1.8; }
...
<div id="questionPage">
  <div class="question-wrap">
    <div class="question-head"><h1>试题练习</h1><p>完成全部选择题后提交，系统会显示正确率、正误和解析；核对后可以重新做题。</p></div>
    <div id="qSummary" class="q-summary"></div>
    <div id="questionList"></div>
  </div>
  <div class="question-actions">
    <button id="redoQuestionsBtn" class="secondary" onclick="redoQuestions()" style="display:none">重新做题</button>
    <button id="submitQuestionsBtn" onclick="submitQuestions()">提交试题</button>
  </div>
</div>
...
function measureNode(node) {
  ...
  if (measured) return { w, h: Math.max(measured, node.level === 1 ? 76 : 62) };
  const noteCount = (node.notes || []).length;
  const imgCount = (node.images || []).length;
  let h = node.level === 1 ? 74 : 50;
  if (node.level === 1) h += estimateTextHeight(node.title || '', 10, 20, 0);
  else if (node.level === 2) h += estimateTextHeight(node.title || '', 14, 21, 0);
  else h += estimateTextHeight(node.title || '', 42, 22, 0);
  if (noteCount) for (const note of node.notes) h += estimateTextHeight(note, 54, 24, 20);
  if (imgCount) h += imgCount * 80;
  return { w, h: Math.max(h, node.level === 1 ? 76 : 62) };
}
...
function shiftSubtree(node, dy) { node.y += dy; (node.children || []).forEach(c => shiftSubtree(c, dy)); }

function preventLevel3Overlap(rootNode) {
  const all = collect(rootNode, []);
  const level3 = all.filter(n => n.level === 3).sort((a, b) => a.y - b.y);
  let bottom = -Infinity;
  const safeGap = 44;
  for (const n of level3) {
    if (n.y < bottom + safeGap) {
      const dy = bottom + safeGap - n.y;
      shiftSubtree(n, dy);
    }
...
function typesetMath(targets=null) {
  if (window.MathJax && MathJax.typesetPromise) {
    if (targets) return MathJax.typesetPromise(targets).catch(() => {});
    return MathJax.typesetPromise().catch(() => {});
  }
  return Promise.resolve();
}

function renderLatexHTML(s) { return escapeHTML(s || ''); }

function waitForImagesInNodes() {
  const imgs = Array.from(document.querySelectorAll('#nodes img'));
...
function render(keepView=false, heightPass=0) {
  document.getElementById('viewport').style.display = '';
  document.getElementById('questionPage').style.display = 'none';
  clozeCounter = 1;
  annotate(tree);
  layout(tree, 40, 40);
  preventLevel3Overlap(tree);
  positioned = collect(tree);
  const maxX = Math.max(...positioned.map(n => n.x + n.w)) + 100;
  const maxY = Math.max(...positioned.map(n => n.y + n.h)) + 120;
  ...
  typesetMath().then(() => waitForImagesInNodes()).then(() => {
...
function openOptions(blank) {
  ...
  menu.innerHTML = options.map(o => `<button type="button" data-v="${escapeHTML(o)}">${renderLatexHTML(o)}</button>`).join('');
...
function clearTypesetElement(el) {
...
function submitAnswers() {
  ...
  document.querySelectorAll('.blank').forEach(b => {
    ...
    if (value === answer) {
      ...
      span.innerHTML = `（${renderLatexHTML(answer)}）`;
...
vp.addEventListener('mousedown', e => {
  if (e.target.closest('.blank') || e.target.closest('#optionMenu') || e.target.closest('.toolbar')) return;
...
window.addEventListener('resize', () => { if (mode !== 'questions') fitView(); });
window.addEventListener('load', () => {
  setTimeout(() => {
    if (mode !== 'questions' && updateMeasuredHeights()) render(false, 0);
  }, 300);
});
