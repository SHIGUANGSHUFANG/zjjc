...
<div class="toolbar">
  ...
  <span class="hint">Heading 4 已合并为说明文字；图片会跟随最近的说明/节点；$...$ 与 $$...$$ 会自动渲染为公式。</span>
</div>
...
function measureNode(node) {{
  ...
  if (noteCount) {{
    for (const note of node.notes) {{
      ...
      if (/\\$\\$[\\s\\S]+?\\$\\$/.test(note) || /\\\\\[[\\s\\S]+?\\\\\]/.test(note)) h += 55;
      else if (/\\$[^$]+\\$/.test(note) || /\\\\\([\\s\\S]+?\\\\\)/.test(note)) h += 16;
    }}
  }}
  if (imgCount) h += 175;
...
function typesetMath() {{
  if (window.MathJax && window.MathJax.typesetPromise) {{
    window.MathJax.typesetPromise([document.getElementById('nodes')]).then(() => {{
      // MathJax may slightly change card height; fit again after formula rendering.
      setTimeout(fitView, 50);
    }}).catch(err => console.warn('MathJax typeset failed:', err));
  }} else {{
    setTimeout(typesetMath, 120);
  }}
}}
function render() {{
